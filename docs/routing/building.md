...
We aim to make Directions API installable on every half-modern hardware, platform and architecture. If the below instructions are not working for you, we'd be happy to help.

### Build Configuration (all platforms)

Directions API uses CMake as build system. When compiling with `gcc` (GNU Compiler Collection), currently version 5 or newer is supported.

Important build options include:

| Option | Behavior |
|--------|----------|
| `-DENABLE_TOOLS` (`On`/`Off`) | Build `directions_api_service` and other utilities (defaults to on)|
...
| `-DENABLE_TESTS` (`On` / `Off`) | Enable Directions API tests (defaults to on)|
...
| `-DENABLE_STATIC_LIBRARY_MODULES` (`On` / `Off`) | If ON builds Directions API modules as STATIC library targets (defaults to off)|
...
```bash
git clone https://github.com/aplog1c/directions-api.git
cd directions-api
...
Now you can build and install Directions API, e.g. 
...
To install Directions API on macOS, you need to install its dependencies with [Homebrew](http://brew.sh):
...
### Include Directions API as a project dependency
...
Currently, `rapidjson`, `date` & `dirent` (Win only) headers are vendored in `third_party`. Consuming applications are encouraged to use `pkg-config` to resolve Directions API and its dependencies which will automatically install those headers to `/path/to/include/valhalla/third_pary/{rapidjson, date, dirent.h}` and can be `#include`d appropriately.

## Running Directions API server on Unix
...
mkdir -p directions_api_tiles
valhalla_build_config --mjolnir-tile-dir ${PWD}/directions_api_tiles --mjolnir-tile-extract ${PWD}/directions_api_tiles.tar --mjolnir-timezone ${PWD}/directions_api_tiles/timezones.sqlite --mjolnir-admin ${PWD}/directions_api_tiles/admins.sqlite > valhalla.json
# build timezones.sqlite to support time-dependent routing
valhalla_build_timezones > directions_api_tiles/timezones.sqlite
...
find directions_api_tiles | sort -n | tar cf directions_api_tiles.tar --no-recursion -T -

# start up the server
directions_api_service valhalla.json 1
